...
# Define the number of frames to consider for average calculation
num_frames_average = 30
...
# Create a window to adjust color
cv2.namedWindow("Color Adjustments")

# Set the new size you want for the window
new_width = 300
new_height = 300

# Resize the window
cv2.resizeWindow("Color Adjustments", new_width, new_height)

# Create trackbars for color change
cv2.createTrackbar("Lower - H", "Color Adjustments", 0, 180, lambda x: None)
cv2.createTrackbar("Lower - S", "Color Adjustments", 0, 255, lambda x: None)
cv2.createTrackbar("Lower - V", "Color Adjustments", 0, 255, lambda x: None)
cv2.createTrackbar("Upper - H", "Color Adjustments", 0, 180, lambda x: None)
cv2.createTrackbar("Upper - S", "Color Adjustments", 0, 255, lambda x: None)
cv2.createTrackbar("Upper - V", "Color Adjustments", 0, 255, lambda x: None)

# Set initial trackbar values
cv2.setTrackbarPos("Lower - H", "Color Adjustments", 35)
cv2.setTrackbarPos("Lower - S", "Color Adjustments", 100)
cv2.setTrackbarPos("Lower - V", "Color Adjustments", 90)
cv2.setTrackbarPos("Upper - H", "Color Adjustments", 77)
cv2.setTrackbarPos("Upper - S", "Color Adjustments", 255)
cv2.setTrackbarPos("Upper - V", "Color Adjustments", 255)
...
# Queue to store dodge signals
dodge_signals = deque(maxlen=DODGE_SIGNAL_FRAME_VALUE)
...
cv2.resizeWindow("dodge_signals", dodge_signals_window_width,
                 dodge_signals_window_height)


def draw_text(image, text, position, color):
    text_size_actual = cv2.getTextSize(
        text, cv2.FONT_HERSHEY_SIMPLEX, text_size, text_thickness)[0]

    text_position = (position[0], position[1] +
                     text_size_actual[1] + text_margin)
...
while True:
    ...
    lower_yellowish_green = np.array([lower_h, lower_s, lower_v])
    upper_yellowish_green = np.array([upper_h, upper_s, upper_v])
    ...
    cv2.line(frame, (middle_point, 0),
             (middle_point, int(height)), (255, 255, 255), 1)
    cv2.line(frame, (0, int(height // 2)),
             (int(width), int(height // 2)), (255, 255, 255), 1)
    cv2.putText(frame, "(0, 0)", (middle_point + 5, int(height // 2) - 5),
    ...
    if len(contours) > 0:
        ...
        if radius > 10:
            ...
            draw_text(frame, "X: {}".format(relative_x),
                      (10, 70), (255, 255, 255))
            draw_text(frame, "Distance: {:.2f} cm".format(
                distance), (10, 100), (255, 255, 255))
            ...
            if len(center_points) >= num_frames_average:
                x_changes = [center_points[i][0] - center_points[i - 1][0]
                             for i in range(1, len(center_points))]
                avg_x_change = sum(
                    x_changes[-num_frames_average:]) / num_frames_average
                avg_distance_change = (
                    distance_values[-1] - distance_values[0]) / num_frames_average

                # Display average X change and distance change
                draw_text(frame, "Avg X Change: {:.2f}".format(
                    avg_x_change), (10, 130), (255, 255, 255))
                draw_text(frame, "Avg Distance Change: {:.2f} cm".format(
                    avg_distance_change), (10, 160), (255, 255, 255))

                # Determine the dodge strategy
                if 40 < distance < 300 and avg_distance_change < -0.8 and distance <= DODGE_THRESHOLD:
                    if -4.2 <= avg_x_change < -1.3:
                        dodge_signal = "Dodge Right!"
                        dodge_signals.appendleft(1)
                        dodge_signal_counter += 1

                    elif 4.2 > avg_x_change > 1.3:
                    ...
                    if dodge_signals_sum > 0 and dodge_signal_counter == 6:
                        main_signal = "Right"
                        if main_signal != last_signal:
                            last_signal = main_signal
                            print(main_signal, DODGE_THRESHOLD,
                                  relative_x, avg_x_change, distance)
                            #ser.write(b'R')

                    elif dodge_signals_sum < 0 and dodge_signal_counter == 6:
                        main_signal = "Left"
                        if main_signal != last_signal:
                            last_signal = main_signal
                            print(main_signal, DODGE_THRESHOLD,
                        ...
                        if relative_x > 0 and avg_x_change > 0:
                            main_signal = "Left(X)"
                            if main_signal != last_signal:
                                last_signal = main_signal
                                print(main_signal, DODGE_THRESHOLD,
                        ...
                        elif relative_x > 0 and avg_x_change < 0:
                            main_signal = "Right(X)"
                            if main_signal != last_signal:
                                last_signal = main_signal
                                print(main_signal, DODGE_THRESHOLD,
    ...
    dodge_signals_str = ", ".join(str(signal) for signal in list(
        dodge_signals)[:DODGE_SIGNAL_FRAME_VALUE])
    dodge_signals_frame = np.zeros(
        (dodge_signals_window_height, dodge_signals_window_width, 3), dtype=np.uint8)
    ...
    draw_text(dodge_signals_frame, "Sum: {}".format(
        dodge_signals_sum), (10, 30), (255, 255, 255))
    draw_text(dodge_signals_frame, "Dodge Signal Counter: {}".format(
        dodge_signal_counter), (10, 60), (255, 255, 255))

    signal_position_y = 120
    for signal in list(dodge_signals)[:DODGE_SIGNAL_FRAME_VALUE]:
        signal_text = str(signal)
        draw_text(dodge_signals_frame, signal_text,
                  (10, signal_position_y), (255, 255, 255))
